...
public class ProductController {
    ...
    void delete(ActionEvent event) {
        ...
        if (selectedProduct == null) {
            ...
            if (productDao.isOrdered(selectedProduct.getProductId())) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Error");
                alert.setHeaderText("You cannot delete products that are ordered by customers.");
                alert.showAndWait();
            } else {
                ...
                if (confirmationResponse.get() == ButtonType.OK) {
                    ...
                    if (result) {
                        dataList.remove(selectedProduct);
        ...
        filterField.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(product -> {
                ...
                if (String.valueOf(product.getProductId()).contains(lowerCaseFilter)) {
                    ...
                    return true;
                } else if (product.getImage().toLowerCase().contains(lowerCaseFilter)) {
                    return true;
                } else {
                    return product.getCategoryName().toLowerCase().contains(lowerCaseFilter);
                }
            });
        });
    ...
    public void initialize() throws SQLException {
        ...
        tcStatus.setCellValueFactory((product) -> {
            return product.getValue().getStatusProperty();
        });
